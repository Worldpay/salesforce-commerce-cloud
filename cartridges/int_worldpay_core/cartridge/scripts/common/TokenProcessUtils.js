...
function addOrUpdateToken(responseData, customerObj, paymentInstrument) {
    ...
    if ((customerObj && responseData.authenticatedShopperID.valueOf().toString() === customerObj.profile.customerNo) || (tokenType)) {
        ...
        var matchedPaymentInstrument = require('int_worldpay_core/cartridge/scripts/common/PaymentInstrumentUtils').getTokenPaymentInstrument(paymentInstruments, paymentInstrument.creditCardNumber, paymentInstrument.creditCardType, paymentInstrument.creditCardExpirationMonth, paymentInstrument.creditCardExpirationYear);
        ...
        if (matchedPaymentInstrument == null) {
            ...
            newPaymentInstrument = require('int_worldpay_core/cartridge/scripts/common/PaymentInstrumentUtils').copyPaymentCardToInstrument(newPaymentInstrument, responseData.cardNumber.valueOf().toString(),
            ...
            Transaction.wrap(function () {
                matchedPaymentInstrument = require('int_worldpay_core/cartridge/scripts/common/PaymentInstrumentUtils').copyPaymentCardToInstrument(matchedPaymentInstrument, null, null, null, null, null, responseData.paymentTokenID.valueOf().toString(), responseData.bin.valueOf().toString());
            });
        }
    }
    return {
...
function addOrUpdateIdentifier(responseData, customerObj, paymentInstrument) {
    ...
    if (matchedPaymentInstrument == null) {
        Transaction.begin();
        var newPaymentInstrument = wallet.createPaymentInstrument(PaymentInstrument.METHOD_CREDIT_CARD);
        newPaymentInstrument = require('int_worldpay_core/cartridge/scripts/common/PaymentInstrumentUtils').copyPaymentCardToInstrumentfortransID(newPaymentInstrument, responseData.cardNumber.valueOf().toString(),
            responseData.cardBrand.valueOf().toString(), new Number(responseData.cardExpiryMonth.valueOf()), new Number(responseData.cardExpiryYear.valueOf()), // eslint-disable-line
    ...
    Transaction.wrap(function () {
        matchedPaymentInstrument = require('int_worldpay_core/cartridge/scripts/common/PaymentInstrumentUtils').copyPaymentCardToInstrumentfortransID(matchedPaymentInstrument, null, null, null, null, null, responseData.transactionIdentifier.valueOf().toString());
    });
