...
function getPayment(wallet) {
    if (wallet) {
        const paymentInstruments = wallet.paymentInstruments;
...
function getCustomerPaymentInstruments(userPaymentInstruments) {
    return userPaymentInstruments.map(function (paymentInstrument) {
        const ccType = paymentInstrument.creditCardType
            || (paymentInstrument.custom && paymentInstrument.custom.awpCardBrand) || 'CREDIT_CARD';

        const masked = paymentInstrument.maskedCreditCardNumber
            || (paymentInstrument.custom && paymentInstrument.custom.awpLast4
                ? '************' + String(paymentInstrument.custom.awpLast4)
                : null);

        const mm = paymentInstrument.creditCardExpirationMonth
            || (paymentInstrument.custom && paymentInstrument.custom.awpExpiryMonth) || null;

        const yy = paymentInstrument.creditCardExpirationYear
            || (paymentInstrument.custom && paymentInstrument.custom.awpExpiryYear) || null;

        const holder = paymentInstrument.creditCardHolder || null;

        let result = {
            creditCardHolder: holder,
            maskedCreditCardNumber: masked,
        ...
        const imgKey = String(ccType || '').toLowerCase().replace(/\s/g, '') || 'credit_card';
        result.cardTypeImage = {
            src: URLUtils.staticURL('/images/' + imgKey + '-dark.svg'),
            alt: ccType
        };
