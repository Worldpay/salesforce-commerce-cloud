    ...
    if (ordersReturnedByQueryIterator.getCount() > 0) {
        ...
        Transaction.wrap(function () {
            while (ordersReturnedByQueryIterator.hasNext()) {
                ...
                var getCreatedOrderDetailsResult = require('link_worldpay_core/cartridge/scripts/pipelets/GetCreatedOrderDetails').getCreatedOrderDetails(orderReturnedByQuery);
                if (getCreatedOrderDetailsResult.success) {
                    // Expression
                    if (getCreatedOrderDetailsResult.toDelete) {
                        OrderMgr.failOrder(orderReturnedByQuery);
                    }
                } else {
                    throw new Error();
                }
