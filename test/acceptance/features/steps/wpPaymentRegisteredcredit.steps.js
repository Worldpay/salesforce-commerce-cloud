...
Then('user selects GooglePay on new ui and make sure no narrative option is present', () => {
  I.click({ xpath: "//label[normalize-space()='Mobile Wallet']" });
  I.wait(2);
  I.dontSeeElement('.form-control.statementNarrative');
  I.dontSee('Purpose of transaction')
});
...
When('Guest Shopper adds new card details', table => {
  for (const id in table.rows) {
    console.log(table.rows);
    if (id < 1) {
      continue; // skip a header of a table
    ...
    // take values
    const PhonenNo = cells[0].value;
    const cardHolderName = cells[1].value;
    const cardNumber = cells[2].value;
    const expMonth = cells[3].value;
    const expYear = cells[4].value;
    const securityCode = cells[5].value;
    worldpayPaymentTestRegisteredcredit.enterCardDetailsGuest(PhonenNo, cardHolderName, cardNumber, expMonth, expYear, securityCode);
    I.wait(5);
  }
});

When('Shopper fills phone number & preferred card for HPP on new ui', table => {
  for (const id in table.rows) {
    console.log(table.rows);
    if (id < 1) {
      continue; // skip a header of a table
    ...
    const preferredCard = cells[1].value;

    worldpayPaymentTestRegisteredcredit.fillPhoneNoReDirectNUI(PhoneNo, preferredCard);

  }
});

When('User adds new card details with save option', table => {
  for (const id in table.rows) {
    console.log(table.rows);
    if (id < 1) {
      continue; // skip a header of a table
    ...
    const expYear = cells[4].value;
    const securityCode = cells[5].value;
    worldpayPaymentTestRegisteredcredit.fillCardDetailsSave(PhonenNo, cardHolderName, cardNumber, expMonth, expYear, securityCode);
    I.wait(5);
  }
});

When('User fills phone number & cvv', table => {
  for (const id in table.rows) {
    console.log(table.rows);
    if (id < 1) {
      continue; // skip a header of a table
    ...
    // take values
    const PhoneNo = cells[0].value;
    const cvv = cells[1].value;

    worldpayPaymentTestRegisteredcredit.fillEmailIDandPhoneNoDirect(PhoneNo, cvv);
  }
});
