...
describe.skip('Checkout - Order confirmation page for Wechat Redirect Payment Method', () => {
    ...
    before(() => {
        return testDataMgr.load()
            ...
            .then(() => browser.waitForVisible('#main #dynamic-content .weChatContent'))
            .then(() => browser.click('#wechatConfirmButton'))
            .then(() => browser.waitForVisible(orderConfPage.RECEIPT_CONTAINER))
            .then(() => browser.pause(500))
            .then(() => browser.getText(orderConfPage.ORDER_NUMBER)
                .then((orderNumber) => {
                    console.log('Required Order Number using Wechat Redirect Payment Method is : ' + orderNumber); // eslint-disable-line
                }));
    });
