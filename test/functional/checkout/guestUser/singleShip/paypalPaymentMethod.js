...
describe('Checkout - Order confirmation page for PayPal Payment Method', () => {
    ...
    before(() => {
        return testDataMgr.load()
            .then(() => {
                const creditCard = testData.creditCardAmex; // eslint-disable-line
                const customer = testDataMgr.getCustomerByLogin(userEmail);

                shippingData = common.createShippingData(customer, locale);
                billingData = common.createBillingData(locale); // eslint-disable-line
            ...
            .then(() => browser.click('#PAYPAL-EXPRESS'))
            .then(() => browser.pause(500))
            ...
            .then(() => browser.waitForVisible('#PMMakePayment'))
            .then(() => browser.click('#PMMakePayment'))
            .then(() => browser.waitForVisible(orderConfPage.RECEIPT_CONTAINER))
            .then(() => browser.pause(500))
            .then(() => browser.getText(orderConfPage.ORDER_NUMBER)
                .then((orderNumber) => {
                    console.log('Required Order Number using Paypal Payment Method is : ' + orderNumber); // eslint-disable-line
                }));
    });
    ...
    describe('Payment information:', () => {
        ...
        it('should display payment method', () => {
            return browser.getText(orderConfPage.PAYMENT_DETAILS)
                .then((paymentDetails) => {
                    const expectedpaymentDetails = 'Payment By Pay Pal\nAmount $132.29';
