...
describe('Checkout - Order confirmation page for Redirect Credit Card Payment Method', () => {
    ...
    before(() => {
        return testDataMgr.load()
            .then(() => {
                ...
                paymentData = common.createredirectPaymentData(creditCard);
                redirectPaymentData = common.createRedirectPaymentData(creditCard);
            ...
            .then(() => browser.waitForExist('#main #dynamic-content'))
            .then(() => checkoutPage.fillOutRedirectPaymentForm(redirectPaymentData))
            .then(() => browser.click('.btn-make-payment#submitButton'))
            .then(() => browser.waitForExist(orderConfPage.RECEIPT_CONTAINER))
            .then(() => browser.waitForVisible(orderConfPage.RECEIPT_CONTAINER))
            .then(() => browser.pause(500))
            .then(() => browser.getText(orderConfPage.ORDER_NUMBER)
                .then((orderNumber) => {
                    console.log('Required Order Number using Redirect Credit Card is : ' + orderNumber); // eslint-disable-line
                }));
    });
    ...
    describe('Payment information:', () => {
        ...
        it('should display payment method', () => {
            return browser.getText(orderConfPage.PAYMENT_DETAILS)
                .then((paymentDetails) => {
                    const expectedpaymentDetails = 'Payment By Credit Card - Redirect\nAmount $132.29';
