...
describe('Checkout - Order confirmation page for Wechat Direct Payment Method', () => {
    ...
    before(() => {
        return testDataMgr.load()
            ...
            .then(() => browser.click('#WECHATPAY-SSL'))
            ...
            .then(() => browser.getText(orderConfPage.ORDER_NUMBER)
                .then((orderNumber) => {
                    console.log('Required Order Number using Wechat Direct Payment Method is : ' + orderNumber); // eslint-disable-line
                }));
    });
    ...
    describe('Payment information:', () => {
        ...
        it('should display payment method', () => {
            return browser.getText(orderConfPage.PAYMENT_DETAILS)
                .then((paymentDetails) => {
                    const expectedpaymentDetails = 'Payment By Wechatpay\nAmount $132.29\nPlease complete your payment using the above QR code for successful order completion\nDo not Press Back, Refresh or Close button in browser\nPlease note, order may get failed if payment is not completed successfully using the QR code';
