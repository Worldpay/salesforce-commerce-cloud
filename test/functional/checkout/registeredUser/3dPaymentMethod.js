...
describe('Checkout - Order confirmation page for 3D Credit Card Payment Method', () => {
    ...
    before(() => {
        return testDataMgr.load()
            .then(() => {
                const creditCard = testData.creditCard3D;
            ...
            .then(() => browser.waitForVisible('.secureWrapper'))
            .then(() => browser.click('.lefty'))
            .then(() => browser.waitForVisible(orderConfPage.RECEIPT_CONTAINER))
            .then(() => browser.pause(500))
            .then(() => browser.getText(orderConfPage.ORDER_NUMBER)
                .then((orderNumber) => {
                    console.log('Required Order Number using 3D Credit Card Payment Method is : ' + orderNumber); // eslint-disable-line
                }));
    });
