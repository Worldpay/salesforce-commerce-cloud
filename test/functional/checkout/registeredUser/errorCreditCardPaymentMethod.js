...
describe('Checkout - Review page for ERROR Credit Card Payment Method', () => {
    ...
    before(() => {
        return testDataMgr.load()
            .then(() => {
                const creditCard = testData.creditCardError;
                ...
                paymentData = common.createFailedPaymentData(creditCard, customer);
            ...
            .then(() => browser.waitForExist(checkoutPage.BTN_ADD_PAYMENT))
            .then(() => browser.waitForVisible(checkoutPage.BTN_ADD_PAYMENT))
            .then(() => browser.click(checkoutPage.BTN_ADD_PAYMENT))
            .then(() => browser.waitForVisible(checkoutPage.PAYMENT_FORM))
            .then(() => browser.pause(500))
            .then(() => checkoutPage.fillOutPaymentForm(paymentData))
            .then(() => common.uncheckCheckbox('.form-check-input:last-child'))
            ...
            .then(() => browser.pause(500))
            .then(() => browser.waitForVisible('.error-message-text'));
    });
    ...
    describe('Error Message', () => {
        it('should display error message', () => {
            return browser.getText('.error-message-text')
                .then((errorMsg) => {
                    console.log('Error Scenario test case using ERROR Credit Card'); // eslint-disable-line
                    const expectedErrorMsg = 'Payment details in the order element are incorrect, please choose a different Payment Method or try again later.';
    ...
    describe('Payment information:', () => {
        ...
        it('should display credit card type', () => {
            return browser.getText('.payment-details span')
                .then((creditCardType) => {
                    const expectedCreditCardType = 'Credit Visa';
                    return assert.equal(creditCardType[0], expectedCreditCardType, 'Expected credit card type = ' + expectedCreditCardType);
                });
        });

        it('should display credit card number', () => {
            return browser.getText('.payment-details div')
                .then((creditCardNumber) => {
                    ...
                    return assert.equal(creditCardNumber[0], expectedCreditCardNumber, 'Expected credit card number = ' + expectedCreditCardNumber);
                });
        });

        it('should display credit card expiration date', () => {
            return browser.getText('.payment-details div span')
                .then((expirationDate) => {
