...
describe('Checkout - Order confirmation page for Sepa Payment Method', () => {
    ...
    before(() => {
        return testDataMgr.load()
            .then(() => {
                ...
                billingData = common.createBillingDataForSepa(locale);
                paymentData = common.createPaymentDataForSepa(customer);
            ...
            .then(() => browser.pause(500))
            .then(() => browser.click('#ELV-SSL'))
            .then(() => browser.pause(500))
            .then(() => browser.waitForVisible(checkoutPage.PAYMENT_FORM))
            .then(() => checkoutPage.fillOutPaymentForm(paymentData))
            .then(() => browser.click('#elvConsent'))
            ...
            .then(() => browser.getText(orderConfPage.ORDER_NUMBER)
                .then((orderNumber) => {
                    console.log('Required Order Number using Sepa Payment Method is : ' + orderNumber); // eslint-disable-line
                }));
    });
    ...
    describe('Payment information:', () => {
        ...
        it('should display payment method', () => {
            return browser.getText(orderConfPage.PAYMENT_DETAILS)
                .then((paymentDetails) => {
                    const expectedPaymentDetails = 'Payment By SEPA\nAmount $132.29';
                    return assert.equal(paymentDetails, expectedPaymentDetails, 'Expected credit card type = ' + expectedPaymentDetails);
                });
        });
    });
