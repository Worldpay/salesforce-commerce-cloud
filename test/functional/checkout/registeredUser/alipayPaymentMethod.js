...
describe('Checkout - Order confirmation page for Alipay Payment Method', () => {
    ...
    before(() => {
        return testDataMgr.load()
            ...
            .then(() => browser.waitForExist(checkoutPage.BTN_NEXT_PLACE_ORDER))
            .then(() => browser.click('#ALIPAY-SSL a'))
            .then(() => browser.pause(500))
            .then(() => browser.waitForVisible(checkoutPage.PAYMENT_FORM))
            ...
            .then(() => browser.selectByValue('select', 'PENDINGOPEN'))
            ...
            .then(() => browser.getText(orderConfPage.ORDER_NUMBER)
                .then((orderNumber) => {
                    console.log('Required Order Number using Alipay Payment Method is : ' + orderNumber); // eslint-disable-line
                }));
    });
    ...
    describe('Payment information:', () => {
        ...
        it('should display payment method', () => {
            return browser.getText(orderConfPage.PAYMENT_DETAILS)
                .then((paymentDetails) => {
                    const expectedpaymentDetails = 'Payment By Alipay\nAmount $132.29';
