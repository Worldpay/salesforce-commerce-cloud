...
describe('Checkout - Order confirmation page for new Customer and Direct Credit Card Payment Method', () => {
    const locale = config.locale;
    const userEmail = Math.random().toString(36).slice(-8) + '@gmail.com';
    ...
    before(() => {
        return testDataMgr.load()
            .then(() => {
                const creditCard = testData.creditCardAmex;
                console.log('New User Email ID :' + userEmail); // eslint-disable-line
                shippingData = common.createShippingDataWithAddress2(locale);
                paymentData = common.createPaymentData(creditCard);
                registeredData = common.createRegisterData(userEmail, password, locale);
            ...
            .then(() => browser.url(config.baseUrl))
            .then(() => browser.waitForVisible('#consent-tracking'))
            .then(() => browser.click('#consent-tracking .affirm'))
            .then(() => browser.pause(500))
            .then(() => browser.click('.user'))
            .then(() => browser.click(loginPage.createAccountTab))
            .then(() => customers.fillOutRegisterForm(registeredData))
            .then(() => browser.click(loginPage.createAccountButton))
            .then(() => browser.pause(500))
            .then(() => browser.waitForVisible('.account-image'))
            .then(() => console.log('In account page')) // eslint-disable-line
            ...
            .then(() => browser.getText(orderConfPage.ORDER_NUMBER)
                .then((orderNumber) => {
                    console.log('Required Order Number for new Customer and Direct Credit Card Payment Method is : ' + orderNumber); // eslint-disable-line
                }));
    });
    ...
    describe('Payment information:', () => {
        ...
        it('should display credit card type', () => {
            return browser.getText(orderConfPage.PAYMENT_CREDIT_TYPE)
                .then((creditCardType) => {
                    const expectedCreditCardType = 'Credit Amex';
        ...
        it('should display credit card number', () => {
            return browser.getText(orderConfPage.PAYMENT_CREDIT_NUMBER)
                .then((creditCardNumber) => {
                    let expectedCreditCardNumber = paymentData[checkoutPage.PAYMENT_CARD_NUMBER].substr(10, 4);
